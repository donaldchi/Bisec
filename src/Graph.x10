...
public class Graph {
    ...
    var degrees:Rail[int];
    var links:ArrayList[int];
    var weights:ArrayList[double];
    var links_r:Rail[ArrayList[Pair[int,double]]];
    ...
    public def this( filename:String, filename_w:String, type_file:int)
    {
        ...
        // Console.OUT.print("nb_nodes = " + nb_nodes + ", ");
        
        degrees = new Rail[int](nb_nodes);
        deg = nb_nodes;
        for (startIndex=0n; startIndex<deg; startIndex++) 
        {
        ...
        total_weight = 0.0;
        for (startIndex=0n; startIndex<nb_nodes; startIndex++) 
        {
            total_weight += weighted_degree(startIndex);    
        }

        // Console.OUT.println("total_weight = " + total_weight);
        nb_nodes = nb_nodes - 1n;
    }

    public def this( filename:String, type_file:int, maxNode:int, direct:int, do_renumber:boolean )
    {
        //This constructer is written for convert function

        Console.OUT.println("In read file part!!");
        val finput = new File(filename);
        var keys:Rail[String];
        var math:Math = new Math();

        links_r = new Rail[ArrayList[Pair[int, double]]](maxNode+1);
        for (startIndex = 0n; startIndex<=maxNode; startIndex++) 
        {
    ...
    public  def this( filename:String, type_file:int, maxNode:int, edgeNum:int)
    {  
        nb_nodes = maxNode;

        degrees = new Rail[int](nb_nodes+1);
        links = new ArrayList[int]();
        weights = new ArrayList[double]();
        ...
        val finput = new File(filename);
        var keys:Rail[String];
        var num:int = 0n; //iterate the edges num from the same src node
    ...
    public def display_binary( filename:String, filename_w:String, type_file:int )
    {
        ...
        deg = Int.operator_as(links_r.size);

        indexs = new Rail[int](deg); //storage index info with degree
        ...
        for (startIndex=0n; startIndex<deg; startIndex++) {
            ...
            indexs(startIndex) = Int.operator_as(links_r(startIndex).size());
            fwriter.writeInt(tot);
        }

        //outputs links
        for (startIndex=0n; startIndex<deg; startIndex++) {
            for (var i:int=0n; i<links_r(startIndex).size(); i++) {
